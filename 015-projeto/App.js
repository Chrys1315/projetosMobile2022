...
export default function App() {
  ...
  const validateAll = () => {
    ...
    if (validateName() == false && validatelastName() == false && validatePassword() == false && validateMail() == false) {
      ...
      setmsgErrText("Há algum campo preenchido iuncorretamente. Verifique!")
    }
  ...
  return (
    <View style={styles.container}>
      ...
      />
      <HelperText style={styles.HelperText} type="error" visible={validatePassword()}>
        O campo senha está inválido!
      </HelperText>
      {/* fim campo senha */}
      {/* inicio campo botao */}
      <Button title="Submit" style={styles.buttonstyle} disabled={btnEnable} color="#6200EE" onPress={() => {validadeAll();}} />
